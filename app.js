...
function showScreen(screenId) {
    ...
    // CRITICAL: Ensure advanced options starts collapsed when showing map
    if (screenId === 'map-overlay') {
        ...
        if (swipeText) {
            swipeText.textContent = '⬆️ More Options';
        }
        // Reset the isExpanded state (it's defined later in initMap but we set intent here)
        if (window.swipeState) {
            window.swipeState.isExpanded = false;
        }
    }
}
...
function updateSiteUI() {
    ...
    const siteLocationEl = document.getElementById('site-location');
    
    if (siteNameEl) siteNameEl.textContent = currentSite.name;
    if (siteLocationEl) siteLocationEl.textContent = currentSite.location.address;
    
    // Update pitch selector
    updatePitchSelector();
    
    // Update current pitch info
    if (currentPitchConfig) {
        document.getElementById('selected-pitch-title').textContent = currentPitchConfig.name;
        document.getElementById('selected-pitch-dims').textContent = 
            `${currentPitchConfig.length} × ${currentPitchConfig.width} yards`;
    }
...
function selectPitch(size) {
    ...
    setTimeout(() => {
        ...
        const centerModeRadio = document.querySelector('input[name="position-mode"][value="center"]');
        const cornerControls = document.getElementById('corner-controls');
        
        if (rotationSlider) rotationSlider.value = 0;
        if (rotationInput) rotationInput.value = 0;
        if (centerModeRadio) centerModeRadio.checked = true;
        if (cornerControls) cornerControls.style.display = 'none';
    }, 0);
...
function initMap() {
    ...
    document.querySelectorAll('input[name="position-mode"]').forEach(radio => {
        radio.addEventListener('change', (e) => {
            positionMode = e.target.value;
            console.log('Position mode changed to:', positionMode);
            ...
            if (positionMode === 'corner') {
                ...
                // Scroll corner controls into view with a delay
                setTimeout(() => {
                    // Get the advanced options panel
                    const advancedPanel = document.getElementById('advanced-options');
                    
                    // Log current scroll position
                    console.log('Before scroll:', {
                        scrollTop: advancedPanel.scrollTop,
                        scrollHeight: advancedPanel.scrollHeight,
                        clientHeight: advancedPanel.clientHeight,
                        maxScroll: advancedPanel.scrollHeight - advancedPanel.clientHeight
                    });
                    
                    // Scroll to the bottom of the panel to show corner buttons
                    const maxScroll = advancedPanel.scrollHeight - advancedPanel.clientHeight;
                    advancedPanel.scrollTop = maxScroll;
                    ...
                    // Also try using scrollTo
                    advancedPanel.scrollTo({
                        top: maxScroll,
                        behavior: 'smooth'
                    });
                }, 300);
                ...
                if (currentLocation) {
                    ...
                    // Highlight active corner button
                    updateCornerButtonStates();
                }
            } else {
    ...
    document.querySelectorAll('.corner-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            lockedCornerType = btn.dataset.corner;
            updateCornerButtonStates();
            
            if (lockedCorner && currentLocation) {
                lockedCorner = [currentLocation.lng, currentLocation.lat];
                const center = calculateCenterFromCorner(lockedCorner, currentPitchConfig, currentRotation, lockedCornerType);
    ...
    // Swipeable advanced options panel
    const swipeHandle = document.getElementById('swipe-handle');
    const advancedOptions = document.getElementById('advanced-options');
    const swipeText = document.querySelector('.swipe-text');
    
    // Make swipe state globally accessible for screen resets
    window.swipeState = window.swipeState || { isExpanded: false };
    
    swipeHandle.addEventListener('click', () => {
        window.swipeState.isExpanded = !window.swipeState.isExpanded;
        
        if (window.swipeState.isExpanded) {
            advancedOptions.classList.remove('collapsed');
            swipeText.textContent = 'Swipe down to hide';
            
            // Debug: Log scroll dimensions
            setTimeout(() => {
                console.log('Panel scroll info:', {
                    scrollHeight: advancedOptions.scrollHeight,
                    clientHeight: advancedOptions.clientHeight,
                    scrollable: advancedOptions.scrollHeight > advancedOptions.clientHeight,
                    scrollDistance: advancedOptions.scrollHeight - advancedOptions.clientHeight
                });
            }, 100);
            ...
            if (scrollHint) {
                advancedOptions.addEventListener('scroll', function hideHint() {
                    if (advancedOptions.scrollTop > 20) {
                        scrollHint.style.display = 'none';
                        advancedOptions.removeEventListener('scroll', hideHint);
                    }
                });
            }
        } else {
            advancedOptions.classList.add('collapsed');
            swipeText.textContent = 'Swipe up for options';
    ...
    function handleSwipe() {
        ...
        // Swipe up (distance > 30px)
        if (swipeDistance > 30 && !window.swipeState.isExpanded) {
            window.swipeState.isExpanded = true;
            advancedOptions.classList.remove('collapsed');
            swipeText.textContent = 'Swipe down to hide';
        }
        // Swipe down (distance < -30px)
        else if (swipeDistance < -30 && window.swipeState.isExpanded) {
            window.swipeState.isExpanded = false;
            advancedOptions.classList.add('collapsed');
            swipeText.textContent = 'Swipe up for options';
        }
...
function updateCornerButtonStates() {
    document.querySelectorAll('.corner-btn').forEach(btn => {
        if (btn.dataset.corner === lockedCornerType) {
