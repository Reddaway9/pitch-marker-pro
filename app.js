...
function updateSiteUI() {
    ...
    // Update site name and location in top center header
    const siteNameEl = document.getElementById('site-name-display');
    const siteLocationEl = document.getElementById('site-location-display');
...
function updatePitchSelector() {
    const selector = document.getElementById('pitch-selector-header');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('pitch-selector-header')?.addEventListener('change', (e) => {
    ...
    document.getElementById('edit-site-name-btn-header')?.addEventListener('click', () => {
        renameSite();
    });
    
    // Corner Lock Toggle Button
    document.getElementById('corner-toggle-btn')?.addEventListener('click', () => {
        const toggleBtn = document.getElementById('corner-toggle-btn');
        const cornerButtons = document.getElementById('corner-buttons-compact');
        
        if (positionMode === 'corner') {
            // Switch to center mode
            positionMode = 'center';
            toggleBtn.classList.remove('active');
            cornerButtons.style.display = 'none';
            console.log('Switched to center mode');
        } else {
            // Switch to corner mode
            positionMode = 'corner';
            toggleBtn.classList.add('active');
            cornerButtons.style.display = 'grid';
    ...
    // Corner Button Selection
    document.querySelectorAll('.corner-btn-compact').forEach(btn => {
        btn.addEventListener('click', () => {
            const corner = btn.dataset.corner;
            lockedCornerType = corner;
            
            if (currentLocation && positionMode === 'corner') {
                lockedCorner = [currentLocation.lng, currentLocation.lat];
                const center = calculateCenterFromCorner(lockedCorner, currentPitchConfig, currentRotation, lockedCornerType);
                createPitchOverlay(center);
            }
...
function initMap() {
    ...
    // Corner Toggle Button (replaces position mode radios)
    const cornerToggleBtn = document.getElementById('corner-toggle-btn');
    const cornerButtonsCompact = document.getElementById('corner-buttons-compact');
    
    if (cornerToggleBtn && cornerButtonsCompact) {
        cornerToggleBtn.addEventListener('click', () => {
            // Toggle corner lock mode
            if (positionMode === 'center') {
                positionMode = 'corner';
                cornerToggleBtn.classList.add('active');
                cornerButtonsCompact.style.display = 'grid';
                
                console.log('Corner lock mode enabled');
                ...
                positionMode = 'center';
                cornerToggleBtn.classList.remove('active');
                cornerButtonsCompact.style.display = 'none';
                lockedCorner = null;
                
                console.log('Center mode enabled');
            }
        });
    }
    
    // Compact Corner selection buttons
    document.querySelectorAll('.corner-btn-compact').forEach(btn => {
        btn.addEventListener('click', () => {
